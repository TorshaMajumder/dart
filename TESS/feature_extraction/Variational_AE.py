...
class VariationalAutoEncoder(object):
    ...
    def __init__(self, X_train=None, epochs=100, batch_size=None, n_filters=1,
                 latent_dim=10, n_neurons=100, lc_type=None, input_dim=1):
...
class GenerateData(object):
    ...
    def __init__(self, lc_type=None, n_filters=1, path=None, columns=None):
...
if __name__ == '__main__':
    ...
    data = GenerateData(lc_type="transients", path="../transients/data/transients.pickle",
                        n_filters=1, columns=['flux', 'flux_err', 'time_phase'])
    X_train, labels, input_shape = data.extract_data()
    vae = VariationalAutoEncoder(X_train=X_train, epochs=50, batch_size=250, latent_dim=10,
                                 lc_type="transients", input_dim=input_shape)
    vae.fit_transform()
    data.save_data()
