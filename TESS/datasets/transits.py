...
def load_latent_space(extract_type=None):
    ...
    try:
        if os.path.exists('../latent_space_data/transits'):

            for file in os.listdir('../latent_space_data/transits'):
                ...
                if extract_type == "k_pca":
                    if f"{extract_type}.pickle" not in file_list:
                        raise FileNotFoundError(f"\nFileNotFoundError: '{extract_type}.pickle' file "
                                                f"doesn't exists in the folder - /latent_space_data/transits.\n")

                    else:
                        with open(f"../latent_space_data/transits/{extract_type}.pickle", 'rb') as file:
            ...
            raise FileNotFoundError(f"\nFileNotFoundError: Data cannot be loaded!"
                                    f"\nPlease verify if the folder - /latent_space_data/transits - exists.\n")
